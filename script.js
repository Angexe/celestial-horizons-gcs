...
function processData(line) {
    ...
    // NOUVEAU : On attend 7 valeurs maintenant (Temp, Press, Alt, Hum, Accel, Lat, Lon)
    if (values.length >= 7) {
        const temp = parseFloat(values[0]);
        const press = parseFloat(values[1]);
        const alt = parseFloat(values[2]);
        const hum = parseFloat(values[3]);  // La nouvelle donnée
        const accel = parseFloat(values[4]);
        ...
        if (!isNaN(temp) && !isNaN(lat)) {
            ...
            document.getElementById('liveHum').innerText = hum.toFixed(1); // NOUVEAU
            ...
            updateChart(humChart, timeStr, hum); // NOUVEAU
            ...
            // NOUVEAU CSV : La colonne Humidité est sauvegardée
